...
const WorkspaceMemberManagement: React.FC = () => {
    ...
    const addMemberMutation = useMutation({
        mutationFn: async (values: { userId: number; role: string }) => {
            const response = await axios.post(`/api/v1/workspaces/${workspaceId}/members`, values);
            return response.data;
        },
        onSuccess: () => {
    ...
    const updateRoleMutation = useMutation({
        mutationFn: async ({ userId, role }: { userId: number; role: string }) => {
            const response = await axios.put(`/api/v1/workspaces/${workspaceId}/members/${userId}`, { role });
            return response.data;
        },
        onSuccess: () => {
    ...
    const handleUpdateRole = (member: WorkspaceMember, newRole: string) => {
